...
export async function POST(request: Request) {
  try {
    ...
    const { data: order, error: orderErr } = await db
      .from('payment_orders')
      .select(
        'id, razorpay_order_id, razorpay_payment_id, tournament_id, status, registration_id, team_invite_id'
      )
    ...
    if (order.team_invite_id) {
      const invite = await loadTeamInviteById(db, order.team_invite_id);
      if (!invite) {
      ...
      if (!result.ok) {
        // Leave the paid order + unpaid invite for admin/manual recovery.
        console.error('Auto-complete team invite registration failed:', result.error);
